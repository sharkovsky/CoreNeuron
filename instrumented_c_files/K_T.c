...
#define initmodel initmodel__K_T
#define _net_receive _net_receive__K_T
#define nrn_state_launcher nrn_state_K_T_launcher
#define nrn_cur_launcher nrn_cur_K_T_launcher
#define nrn_jacob_launcher nrn_jacob_K_T_launcher 
#define rates rates_K_T 
#define states states_K_T 
...
extern "C" {
 ...
 #if defined(PG_ACC_BUGS)
#define _celsius_ _celsius__K_T
double _celsius_;
...
#define hTauF hTauF_K_T
 double hTauF = 1;
 #pragma acc declare copyin (hTauF)
#define mTauF mTauF_K_T
 double mTauF = 1;
 #pragma acc declare copyin (mTauF)
#define vshift vshift_K_T
 double vshift = 0;
 #pragma acc declare copyin (vshift)
 
static void _acc_globals_update() {
 #pragma acc update device (hTauF) if(nrn_threads->compute_gpu)
 #pragma acc update device (mTauF) if(nrn_threads->compute_gpu)
 #pragma acc update device (vshift) if(nrn_threads->compute_gpu)
 }
...
#define _slist1 _slist1_K_T
int* _slist1;
#pragma acc declare create(_slist1)

#define _dlist1 _dlist1_K_T
int* _dlist1;
...
LIKWID_MARKER_START("K_T_current");
#endif
_PRAGMA_FOR_VECTOR_LOOP_
...
#ifndef DISABLE_LIKWID_ON_MECH
LIKWID_MARKER_STOP("K_T_current");
#endif
