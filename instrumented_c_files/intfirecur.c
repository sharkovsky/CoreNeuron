...
#define nrn_init _nrn_init__IntFireCur
#define nrn_cur _nrn_cur__IntFireCur
#define _nrn_current _nrn_current__IntFireCur
#define nrn_jacob _nrn_jacob__IntFireCur
#define nrn_state _nrn_state__IntFireCur
#define initmodel initmodel__IntFireCur
#define _net_receive _net_receive__IntFireCur
#define nrn_state_launcher nrn_state_IntFireCur_launcher
#define nrn_cur_launcher nrn_cur_IntFireCur_launcher
#define nrn_jacob_launcher nrn_jacob_IntFireCur_launcher 
...
#define refrac _p[1*_STRIDE]
#define minf _p[2*_STRIDE]
#define gid _p[3*_STRIDE]
#define m _p[4*_STRIDE]
#define t0 _p[5*_STRIDE]
#define on _p[6*_STRIDE]
#define cnt _p[7*_STRIDE]
...
#endif /*BBCORE*/
#define M M_IntFireCur
#define firetime firetime_IntFireCur
 inline double M( _threadargsproto_ );
 inline double firetime( _threadargsproto_ );
...
#endif
 {
   ...
   if ( _lflag  == 1.0 ) {
     if ( m >= .999999 ) {
       ...
       m = 0.0 ;
       on = 0.0 ;
       artcell_net_send ( _tqitem, _weight_index, _pnt, t +  refrac + 1e-6 , 2.0 ) ;
       }
     else {
       artcell_net_send ( _tqitem, _weight_index, _pnt, t +  firetime ( _threadargs_ ) , 1.0 ) ;
       }
     }
   else if ( _lflag  == 2.0 ) {
     on = 1.0 ;
...
static void initmodel(_threadargsproto_) {
  int _i; double _save;{
 {
   ...
   t0 = t ;
   on = 1.0 ;
   artcell_net_send ( _tqitem, -1, _nt->_vdata[_ppvar[1*_STRIDE]], t +  firetime ( _threadargs_ ) , 1.0 ) ;
   }
