...
#define initmodel initmodel__DetGABAAB
#define _net_receive _net_receive__DetGABAAB
#define nrn_state_launcher nrn_state_DetGABAAB_launcher
#define nrn_cur_launcher nrn_cur_DetGABAAB_launcher
#define nrn_jacob_launcher nrn_jacob_DetGABAAB_launcher 
#if NET_RECEIVE_BUFFERING
#define _net_buf_receive _net_buf_receive_DetGABAAB
static void _net_buf_receive(_NrnThread*);
#endif
 
#define state state_DetGABAAB 
...
#define R _p[19*_STRIDE]
#define Pr _p[20*_STRIDE]
#define u _p[21*_STRIDE]
#define tsyn _p[22*_STRIDE]
#define A_GABAA _p[23*_STRIDE]
#define B_GABAA _p[24*_STRIDE]
#define A_GABAB _p[25*_STRIDE]
#define B_GABAB _p[26*_STRIDE]
#define factor_GABAA _p[27*_STRIDE]
#define factor_GABAB _p[28*_STRIDE]
#define DA_GABAA _p[29*_STRIDE]
#define DB_GABAA _p[30*_STRIDE]
#define DA_GABAB _p[31*_STRIDE]
#define DB_GABAB _p[32*_STRIDE]
#define _v_unused _p[33*_STRIDE]
#define _g_unused _p[34*_STRIDE]
...
#define _slist1 _slist1_DetGABAAB
int* _slist1;
#pragma acc declare create(_slist1)

#define _dlist1 _dlist1_DetGABAAB
int* _dlist1;
...
static void _net_buf_receive(_NrnThread* _nt) {
  ...
  _nrb->_displ_cnt = 0;
  _nrb->_cnt = 0;
  /*printf("_net_buf_receive__DetGABAAB  %d\n", _nt->_id);*/
...
#error AoSoA not implemented.
 ...
 #endif 
 _tsav = t; {
   double _lresult , _lweight_GABAA , _lweight_GABAB ;
 _lweight_GABAA = _args[0] ;
   _lweight_GABAB = _args[0] * GABAB_ratio ;
   if ( Fac > 0.0 ) {
     u = u * exp ( - ( t - tsyn ) / Fac ) ;
     }
   else {
   ...
   tsyn = t ;
   A_GABAA = A_GABAA + Pr * _lweight_GABAA * factor_GABAA ;
   B_GABAA = B_GABAA + Pr * _lweight_GABAA * factor_GABAA ;
   A_GABAB = A_GABAB + Pr * _lweight_GABAB * factor_GABAB ;
   B_GABAB = B_GABAB + Pr * _lweight_GABAB * factor_GABAB ;
   if ( verboseLevel > 0.0 ) {
...
LIKWID_MARKER_START("DetGABAAB_EMS_current");
#endif
...
#ifndef DISABLE_LIKWID_ON_SYN
LIKWID_MARKER_STOP("DetGABAAB_EMS_current");
#endif
#ifndef DISABLE_LIKWID_ON_SYN
LIKWID_MARKER_START("DetGABAAB_EMS_reduction_cur");
#endif
#ifdef _OPENACC
    if(!(_nt->compute_gpu)) { 
        for (_iml = 0; _iml < _cntml_actual; ++_iml) {
...
#ifndef DISABLE_LIKWID_ON_SYN
LIKWID_MARKER_STOP("DetGABAAB_EMS_reduction_cur");
#endif
...
#ifndef DISABLE_LIKWID_ON_SYN
LIKWID_MARKER_START("DetGABAAB_EMS_state");
#endif
#pragma omp simd simdlen(2)
for (_iml = 0; _iml < _cntml_actual; ++_iml) {
#else /* LAYOUT > 1 */ /*AoSoA*/
...
#ifndef DISABLE_LIKWID_ON_SYN
LIKWID_MARKER_STOP("DetGABAAB_EMS_state");
#endif
}
